...
class CreateModelsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
       ...
       $ipatch = !empty($ipatch)?$ipatch.'/':'';

       if(!is_dir('app/model/'.$ipatch))
          mkdir('app/model/'.$ipatch);
    ...
    private function createRestService($name, OutputInterface $output)
    {
        if (!file_exists('app/service/'.$name.'Service.class.php')) {
            ...
            $contents = str_replace('$database', 'sample', $contents);

            if (false === file_put_contents('app/service/'.$name.'Service.class.php', $contents)) {
